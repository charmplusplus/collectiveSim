...
avg_msg_pass = [x / 2 for x in avg_msg_pass]  # Convert to per message
avg_zero_copy = [x / 2 for x in avg_zero_copy]  # Convert to per message
...
ax.set_ylabel('Average Time Per message(micro seconds)')
ax.set_title('Message Passing vs. Zero Copy (nodes = 1, PE/node = 2)')
